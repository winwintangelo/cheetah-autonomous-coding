...
COMMANDS_NEEDING_EXTRA_VALIDATION = {"pkill", "chmod", "init.sh", "find", "bash", "sh"}
...
def validate_shell_command(command_string: str) -> tuple[bool, str]:
    """
    Validate bash/sh commands - block dangerous flags like -c.
    
    Security restrictions:
    1. Block -c flag (allows arbitrary command execution)
    2. Only allow direct script execution: bash script.sh
    ...
    if not tokens or tokens[0] not in ("bash", "sh"):
        return False, "Not a bash/sh command"
    
    # Block dangerous flags
    dangerous_flags = {"-c", "--command"}
    for token in tokens[1:]:
        if token in dangerous_flags:
            return False, f"'{token}' flag is not allowed (could bypass command validation)"
    
    # Must have at least a script argument
    if len(tokens) < 2:
        return False, "bash/sh requires a script file argument"
    ...
    for cmd in commands:
        ...
        if cmd in COMMANDS_NEEDING_EXTRA_VALIDATION:
            ...
            if cmd == "pkill":
                ...
                allowed, reason = validate_shell_command(cmd_segment)
                if not allowed:
                    return {"decision": "block", "reason": reason}
