...
try:
    from logging_util import log, log_thinking, get_timestamp
except ImportError:
    ...
    def log_thinking(duration_ms: float) -> None:
        print(f"\n[{get_timestamp()}] [Thinking: {duration_ms/1000:.1f}s]", flush=True)
...
CODING_TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "List files in multiple directories at once (more efficient than multiple list_directory calls)",
            "parameters": {
                "type": "object",
                "properties": {
                    "paths": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "Array of directory paths to list (e.g., ['.', 'src', 'server'])"
                    }
                },
                "required": ["paths"]
            }
...
class OpenRouterAgent(BaseCodingAgent):
    ...
    def _validate_command(self, command: str) -> tuple[bool, str]:
        ...
        # Handle ./ prefixed scripts (e.g., ./init.sh)
        if base_cmd.startswith("./"):
            script_name = base_cmd[2:]  # Remove ./
            if script_name in self.config.allowed_commands:
                return True, ""
            # Also check without extension for flexibility
            script_base = script_name.rsplit(".", 1)[0] if "." in script_name else script_name
            if script_base in self.config.allowed_commands:
                return True, ""
            return False, f"Script '{base_cmd}' not in allowed list"
    ...
    async def _execute_tool(self, tool_name: str, arguments: dict) -> str:
        ...
        try:
            if tool_name == "read_file":
            ...
            elif tool_name == "batch_list_directories":
                paths = arguments.get("paths", [])
                if not paths:
                ...
                for path_arg in paths:
                    ...
                    elif not str(dir_path.resolve()).startswith(str(project_dir.resolve())):
                        results.append(f"=== {path_arg} ===\nError: Access denied - directory outside project directory")
                    else:
                        files = list(dir_path.iterdir())
                        if not files:
                            results.append(f"=== {path_arg} ===\n(empty directory)")
