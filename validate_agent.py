...
Validates that the coding agent is properly configured and ready to use.
...
Options:
    ...
    --commands   Run comprehensive tool/command tests
...
def run_command_tests() -> tuple[int, int]:
    """Run command tests for the agent."""
    ...
    test_dir = Path(tempfile.mkdtemp(prefix="cmd_test_"))
...
def run_live_test() -> tuple[int, int]:
    """Run a live test with the agent."""
...
def print_summary(results: dict[str, tuple[int, int]]) -> None:
    ...
    for section, (passed, total) in results.items():
        ...
        print(f"  {section}: {passed}/{total} checks passed - {status}")
...
{Colors.BOLD}Usage:{Colors.END}
  python validate_agent.py                   # Validate setup
  python validate_agent.py --test            # Run live test (uses API credits)
  python validate_agent.py --commands        # Run command tests
...
{Colors.BOLD}Options:{Colors.END}
  ...
  --commands  Run comprehensive command/tool tests (~35 tests)
...
def main():
    ...
    # Check OpenRouter agent
    passed, total = validate_openrouter_agent()
    results["OpenRouter"] = (passed, total)
    
    # Run live test if requested
    if run_test:
        passed, total = run_live_test()
        results["Live Test"] = (passed, total)
    
    # Run command tests if requested
    if run_cmd_tests:
        passed, total = run_command_tests()
        results["Command Tests"] = (passed, total)
